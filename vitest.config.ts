 ...
 * monorepo (see docs/ctap-intake/README.md). A few boundary modules it imports
 * (`../gmail`, `../pdf`, `../../../drizzle/schema`) live in that monorepo and are
 * intentionally not synced here. To exercise the synced logic standalone we
 * resolve those specifiers to lightweight test doubles WITHOUT adding files into
 * the `sync/` tree, so the patch stays clean for cherry-picking.
 *
 * The synced tests never invoke these doubles' behavior — they only need the
 * imported symbols to exist so the modules load.
 */
